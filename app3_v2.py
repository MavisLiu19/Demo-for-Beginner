...
# load the model.pkl
path = r"E:\NUS-EBA课程资料\Semester 2\Data Science Solution Implementation\Day-2 (Prakash)\code files\python\ap3-main\model.pkl"
...
@st.cache_data()
def prediction(int_rate, emp_length, annual_inc, delinq_2yrs, 
	fico_range_high,revol_bal, open_acc):
	# Making predictions
	prediction = model.predict([[int_rate, emp_length, annual_inc, delinq_2yrs,fico_range_high,revol_bal, open_acc]])
	if prediction == 0:
		pred = 'This loan is Rejected'
	else:
		pred = 'This loan is Approved'
	return pred
...
def main():
   ...
   ###################### declare inputs for UI #####################
	# define/create the left handside of the app interface (refer to the app interface)
	int_rate = st.sidebar.slider('int_rate', 5, 25, 10) # 10 is the default value
	st.write('input int_rate', int_rate) # show what you have choose in int_rate on the right side of the interface
